...
Writes ``report/wc2026_report.html``.
"""
...
OUT_HTML = HERE / "report" / "wc2026_report.html"
SEED = 42
...
def _load_table(sheet, csv_name):
    if XLSX.exists():
...
def load_matches():
    df = _load_table("matches", "matches_raw.csv")
...
def load_teams():
    df = _load_table("teams", "teams_raw.csv")
...
def load_players():
    df = _load_table("players", "players_raw.csv")
    df["team"] = df["team"].map(canon)
